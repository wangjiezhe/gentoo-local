# Copyright 2003-2025 Gentoo Authors
# Distributed under the terms of the GNU General Public License v2

EAPI=8

PYTHON_COMPAT=( python3_{11..13} )
DISTUTILS_USE_PEP517=no
DISTUTILS_EXT=1
inherit distutils-r1

CUDA_VER="12.9"
BASE_VER="$(ver_cut 1-3)"
EXTEND_VER="${BASE_VER}"

DESCRIPTION="An SDK for high-performance deep learning inference"
HOMEPAGE="https://developer.nvidia.com/tensorrt"
SRC_URI="https://developer.nvidia.com/downloads/compute/machine-learning/tensorrt/${BASE_VER}/tars/TensorRT-${PV}.Linux.x86_64-gnu.cuda-${CUDA_VER}.tar.gz"
S="${WORKDIR}/TensorRT-${PV}"

LICENSE="NVIDIA-TensorRT"
SLOT="0/${BASE_VER}"
KEYWORDS="~amd64"
RESTRICT="bindist mirror test"

RDEPEND="
	=dev-util/nvidia-cuda-toolkit-12*
	dev-libs/cudnn
	$(python_gen_cond_dep '
		>=dev-python/gpep517-15[${PYTHON_USEDEP}]
	')
"
BDEPEND="
	dev-util/patchelf
"

QA_PREBUILT="/opt/cuda/targets/x86_64-linux/lib/*"

python_install() {
	local PYTHON_VER="${EPYTHON/python/}"
	local PYTHON_VER="${PYTHON_VER/./}"
	distutils_wheel_install "${ED}" "${S}/python/tensorrt-${PV}-cp${PYTHON_VER}-none-linux_x86_64.whl"
	distutils_wheel_install "${ED}" "${S}/python/tensorrt_dispatch-${PV}-cp${PYTHON_VER}-none-linux_x86_64.whl"
	distutils_wheel_install "${ED}" "${S}/python/tensorrt_lean-${PV}-cp${PYTHON_VER}-none-linux_x86_64.whl"
}

python_install_all() {
	into /opt/cuda
	dobin bin/*

	insinto /opt/cuda/targets/x86_64-linux/
	doins -r include lib

	for suffix in {win_,}{sm75,sm80,sm86,sm89,sm90,sm120,ptx} sm100; do
		dosym "libnvinfer_builder_resource_${suffix}.so.${BASE_VER}" \
		"/opt/cuda/targets/x86_64-linux/lib/do_not_link_against_nvinfer_builder_resource_${suffix}"
	done

	patchelf --add-rpath '$ORIGIN' "${ED}/opt/cuda/targets/x86_64-linux/lib/libnvinfer.so"|| die "patchelf failed"
}

src_install() {
	distutils-r1_src_install
}
