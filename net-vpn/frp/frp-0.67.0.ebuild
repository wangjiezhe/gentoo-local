...
src_prepare() {
	default
	if use web; then
		cp -r "${WORKDIR}/node_modules" "${WORKDIR}/${P}/web/frpc"
		cp -r "${WORKDIR}/node_modules" "${WORKDIR}/${P}/web/frps"
	fi
}
...
src_compile() {
	...
	use web || sed -i '/web\/frp/d' cmd/frp{c,s}/main.go || die

	if use client; then
		use web && emake -C web/frpc build
	...
	if use server; then
		use web && emake -C web/frps build
